...
@BugPattern(name = "Slf4jDoNotLogMessageOfExceptionExplicitly", summary = "Do not log message returned from Throwable#getMessage and Throwable#getLocalizedMessage", tags = {
		"SLF4J" }, severity = ERROR)
...
public class ManuallyProvidedMessage extends BugChecker implements MethodInvocationTreeMatcher {
	...
	public Description matchMethodInvocation(MethodInvocationTree tree, VisitorState state) {
		...
		Optional<JCFieldAccess> problem = tree.getArguments().stream()
				.filter(arg -> arg.getClass().isAssignableFrom(JCMethodInvocation.class))
				.map(JCMethodInvocation.class::cast).map(arg -> arg.meth)
				.filter(meth -> meth.getClass().isAssignableFrom(JCFieldAccess.class)).map(JCFieldAccess.class::cast)
				.filter(meth -> MESSAGE_GETTER.or(LOCALIZED_MESSAGE_GETTER).test(meth.sym.name.toString()))
				.filter(meth -> THROWABLE.test(meth.sym.owner.toString())).findFirst();
		if (problem.isPresent()) {
			return Description.builder(tree, "Slf4jDoNotLogMessageOfExceptionExplicitly",
					"https://github.com/KengoTODA/findbugs-slf4j#slf4j_manually_provided_message", ERROR,
